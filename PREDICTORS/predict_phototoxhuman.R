...
path <- paste(FPADMET_PATH, "/MODELS/model_phototoxhuman_pubchem.rds", sep="")
fittedmodel <- readRDS(path)
#
# fittedmodel <- readRDS("MODELS/model_phototoxhuman_pubchem.rds")
...
if (applyadan) {
    ...
    confidence = 1 - apply(pValues, 1, FUN=min) # for binary
