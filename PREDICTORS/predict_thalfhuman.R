...
path <- paste(FPADMET_PATH, "/MODELS/model_thalfhuman_asp.rds", sep="")
fittedmodel <- readRDS(path)

# fittedmodel <- readRDS("MODELS/model_thalfhuman_asp.rds")
...
if (applyadan) {
    ...
    confidence = 1 - apply(pValues, 1, FUN=secondLargest) 
